...
class Decoder(nn.Module):
    def __init__(self, z_dim, h_dim, output_dim):
        ...
        self.layer_mu = nn.Linear(h_dim, output_dim)
        self.layer_sigma = nn.Linear(h_dim, output_dim)

    def forward(self, z):
        h = F.tanh(self.layer1(z))
        mu, sigma = self.layer_mu(h), self.layer_sigma(h)
        return mu, sigma
...
class VAE(nn.Module):
    def __init__(self, input_dim, h_dim, z_dim, output_dim):
        super().__init__()
        self.encoder = Encder(input_dim, h_dim, z_dim)
    ...
    def forward(self, x):
        ...
        output = np.random.normal(mu, sigma, self.output_dim)
        return output
